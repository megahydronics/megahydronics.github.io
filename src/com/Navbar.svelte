                ...
                <div class="hidden md:ml-10 md:flex md:space-x-5 md:items-center">
                    {#each pages as page, index}
                        <button
                            ...
                            class="px-2 py-1 text-sm font-medium {index == $pageIndex ? 'text-gray-900' : 'text-gray-400  hover:text-gray-500'}">{page.name}</button>
                    {/each}
                </div>
            </div>
            <div class="hidden gap-x-4 gap-y-2 text-sm justify-center md:flex md:flex-row md:items-center">
                {#each contacts as contact, index}
                    <a data-label="contacts.{index}" href={contact.link} class="flex items-center gap-x-2 text-gray-400 hover:text-gray-500 p-1">
    ...
    <div class="md:hidden {menuOpen ? 'block' : 'hidden'}" id="mobile-menu">
        <div class="space-y-1 pb-3 pt-2">
            {#each pages as page, index}
                <button
                    on:click={pageIndexSetter(index)}
                    class="block w-full py-2 pl-3 pr-4 text-base font-medium {index == $pageIndex ? 'bg-indigo-50 text-indigo-700' : 'text-gray-400 hover:bg-gray-50  hover:text-gray-500'}"
                    >{page.name}</button>
            {/each}
        </div>
        <div class="border-t border-gray-200 pb-3 pt-3">
            <div class="space-y-1">
                {#each contacts as contact, index}
                    <a href={contact.link} class="flex whitespace-nowrap justify-center h-10 items-center gap-x-3 text-gray-400 hover:bg-gray-100 hover:text-gray-500">
                        <Icon index={contact.icon} size={5} />
