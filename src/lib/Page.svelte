    ...
    <div class="flex h-16 justify-between">
      <div class="flex">
        ...
        <div class="hidden md:ml-10 md:flex md:space-x-8">
          {#each Object.values(pageInfos) as info}
            <a
              href={info.path}
      ...
      >
        {#each Object.values(contactInfos) as info}
          <div
  ...
  <div class="md:hidden {menuOpen ? 'block' : 'hidden'}" id="mobile-menu">
    <div class="space-y-1 pb-3 pt-2">
      {#each Object.values(pageInfos) as info}
        <a
          href={info.path}
          class="block border-l-4 py-2 pl-3 pr-4 text-base font-medium {info.id == pageId
...
<footer class="mt-auto">
  <div class="mx-auto max-w-7xl px-6 py-8 md:flex md:items-center md:justify-between lg:px-8">
    <div class="flex justify-center space-x-6 md:order-2">
      {#each Object.values(socialInfos) as info}
        <a href={info.url} class="text-gray-400 hover:text-gray-500" target="_blank" rel="noopener noreferrer">
          <span class="sr-only">{info.title}</span>
          <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path fill-rule="evenodd" d={info.svgPath} clip-rule="evenodd" />
          </svg>
        </a>
      {/each}
    </div>
